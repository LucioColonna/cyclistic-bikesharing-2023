...
skimmer <- skim_without_charts(df) %>%
    ...
    mutate(
      ...
      percentage_not_available = round(total_empty_or_NA / nrow(df) * 100, 2),
      percentage_available = 100 - percentage_not_available
    ) %>%
...
skimmer_long <- skimmer %>%
  mutate(has_missing = percentage_not_available > 0) %>%
  pivot_longer(
    cols = c(percentage_not_available, percentage_available),
    names_to = "status",
    values_to = "percentage"
  ) %>%
  mutate(
    status = factor(status, levels = c("percentage_available", "percentage_not_available")),
    label = if_else(
      status == "percentage_not_available" & has_missing, 
...
ggplot(skimmer_long, aes(x = skim_variable, y = percentage, fill = status)) +
  ...
  scale_fill_manual(
    ...
    values = c("percentage_available" = "lightgreen", "percentage_not_available" = "red"),
